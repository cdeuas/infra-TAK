...
**How to apply on existing servers:** In the infra-TAK console on the TAK Server page, click **"Resync LDAP to TAK Server"**. This re-runs the full LDAP flow, re-patches `CoreConfig.xml` with the full `<auth>` block (including the new attribute), and restarts TAK Server. Existing certs and users are not affected — only the auth block is rewritten.
