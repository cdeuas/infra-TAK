...
Mesh IP: Dynamically assigned on wt0 via NetBird (Current: 100.112.85.17).
...
- Unified HTTP mode for 100% offline MANET operations.
...
🚀 Project ARES (Current Stable)
- **Domain**: `ares.local`
- **DNS**: Wildcard resolution via mesh-native `dnsmasq` on VM (100.112.85.17:53).
- **Gateway**: Caddy bound to poort 80 of `wt0`. All services accessible via `*.ares.local`.
